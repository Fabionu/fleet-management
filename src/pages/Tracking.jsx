...
{/* ── Client / Comandă ── */}
                    ...
                    <div style={{ padding: '5px 14px', flex: 1, borderRight: '1px solid var(--gray-2)', display: 'flex', flexDirection: 'column' }}>
                      {truck.unload_location ? (
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '1px' }}>
                            <div style={{ fontSize: '13px', color: 'var(--black)' }}>{truck.unload_location}</div>{(() => { try { const _s=JSON.parse(truck.extra_stops||'[]'); const _n=_s.filter(x=>x.type==='unload').length; return _n>0 ? <span style={{ fontSize:'10px', fontWeight:700, background:'rgba(255,122,61,0.12)', color:'#ff7a3d', padding:'1px 6px', borderRadius:'4px', marginLeft:'4px', whiteSpace:'nowrap' }}>+{_n}</span> : null; } catch { return null; } })()}
