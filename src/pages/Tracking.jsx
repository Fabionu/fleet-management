                        ...
                        <div data-weekend-form onMouseDown={e => e.stopPropagation()} style={{ position: 'relative' }}>
                          ...
                          {user?.permissions?.editWeekendHistory && weekendEditId !== truck.id && (
                            <button
                              ...
                              style={{ marginTop: 3, width: '100%', padding: '3px 0', background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, color: 'var(--gray-3)', fontSize: 10, fontWeight: 500, transition: 'color 0.15s' }}
                              onMouseEnter={e => { e.currentTarget.style.color = 'var(--gray-4)'; }}
...
{/* ── Client / Comandă ── */}
                      ...
                      {truck.load_location ? (
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '1px' }}>
                            <div style={{ fontSize: '13px', color: 'var(--black)' }}>{truck.load_location}</div>{(() => { try { const _stops=JSON.parse(truck.extra_stops||'[]').filter(x=>x.type==='load'); const _k=`${truck.id}-load`; return _stops.length>0 ? <span onClick={(e) => openStopsPopover(e,_k,_stops)} style={{ fontSize:'10px', fontWeight:700, background: stopsPopover?.key===_k ? '#ff7a3d' : 'rgba(255,122,61,0.12)', color: stopsPopover?.key===_k ? '#fff' : '#ff7a3d', padding:'1px 6px', borderRadius:'4px', marginLeft:'4px', whiteSpace:'nowrap', cursor:'pointer', userSelect:'none' }}>+{_stops.length}</span> : null; } catch { return null; } })()}
                    ...
                    <div style={{ padding: '5px 14px', flex: 1, borderRight: '1px solid var(--gray-2)', display: 'flex', flexDirection: 'column' }}>
                      {truck.unload_location ? (
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '1px' }}>
                            <div style={{ fontSize: '13px', color: 'var(--black)' }}>{truck.unload_location}</div>{(() => { try { const _stops=JSON.parse(truck.extra_stops||'[]').filter(x=>x.type==='unload'); const _k=`${truck.id}-unload`; return _stops.length>0 ? <span onClick={(e) => openStopsPopover(e,_k,_stops)} style={{ fontSize:'10px', fontWeight:700, background: stopsPopover?.key===_k ? '#ff7a3d' : 'rgba(255,122,61,0.12)', color: stopsPopover?.key===_k ? '#fff' : '#ff7a3d', padding:'1px 6px', borderRadius:'4px', marginLeft:'4px', whiteSpace:'nowrap', cursor:'pointer', userSelect:'none' }}>+{_stops.length}</span> : null; } catch { return null; } })()}
              ...
              return (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '2px', paddingBottom: i < stopsPopover.stops.length - 1 ? '8px' : 0, marginBottom: i < stopsPopover.stops.length - 1 ? '8px' : 0, borderBottom: i < stopsPopover.stops.length - 1 ? '1px solid var(--gray-2)' : 'none' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                    {s.coords && <span style={{ fontSize: '11px', color: 'var(--gray-4)', fontFamily: 'monospace' }}>{s.coords}</span>}
                    <button
                      type="button"
                      onClick={() => { if (!hasCoords) return; navigator.clipboard.writeText(s.coords).then(() => { setCopiedKey(stopKey); setTimeout(() => setCopiedKey(null), 2000); }); }}
